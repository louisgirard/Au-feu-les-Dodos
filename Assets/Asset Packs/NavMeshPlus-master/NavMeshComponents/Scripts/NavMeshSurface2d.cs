    ...
    public class NavMeshSurface2d : MonoBehaviour
    {
        ...
        void AppendModifierVolumes(ref List<NavMeshBuildSource> sources)
        {
            ...
            foreach (var m in modifiers)
            {
                ...
                src.size = msize;
                src.area = m.area;
                Debug.Log("Volume: center = " + mcenter + ", size = " + src.size + ", transform = " + src.transform);
                sources.Add(src);
            }
        }
        ...
        void AppendBoxCollisions(ref List<NavMeshBuildSource> sources)
        {
#if UNITY_EDITOR
            var myStage = StageUtility.GetStageHandle(gameObject);
            if (!myStage.IsValid())
            ...
            foreach (var m in modifiers)
            {
                ...
                BoxCollider2D collider = m.GetComponent<BoxCollider2D>();
                ...
                if (!myStage.Contains(m.gameObject))
                    continue;
#endif
                var mcenter = collider.bounds.center;
                var scale = m.transform.lossyScale;
                var msize = new Vector3(collider.size.x * Mathf.Abs(scale.x), collider.size.y * Mathf.Abs(scale.y), 1);
